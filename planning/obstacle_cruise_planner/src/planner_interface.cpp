  ...
  const PlannerData & planner_data, const std::vector<StopObstacle> & stop_obstacles)
{
  ...
  debug(candidate_stop_dist_on_ref_traj);
  ...
  if (closest_behavior_stop_idx) {
    debug(*closest_behavior_stop_idx);
    ...
    if (*closest_behavior_stop_idx == planner_data.traj_points.size() - 1) {
      std::cerr << "check goal point" << std::endl;
      // Closest behavior stop point is the end point
      if (
        stop_dist_diff < longitudinal_info_.safe_distance_margin &&
        stop_dist_diff > longitudinal_info_.terminal_safe_distance_margin) {
        std::cerr << "modifiy stop dist" << std::endl;
        ...
        candidate_stop_dist_on_ref_traj += longitudinal_info_.safe_distance_margin -
                                           longitudinal_info_.terminal_safe_distance_margin;
      }
    } else {
